...
use crate::{
    ...
    indexer::{
        ...
        fetch_logs::{fetch_logs_stream, FetchLogsResult, LiveIndexingConfig},
...
pub async fn process_event(
    ...
    process_event_logs(Arc::new(config), None, block_until_indexed).await?;
...
async fn process_event_logs(
    ...
    let mut logs_stream =
        fetch_logs_stream(Arc::clone(&config), live_indexing_configs, block_until_indexed);
...
async fn process_contract_events_with_dependencies(
    ...
    let mut live_indexing_configs = Vec::new();

    while let Some(current_tree) = stack.pop() {
        for dependency in &current_tree.contract_events {
            let event_processing_config = events_processing_config
                .iter()
                .find(|e| {
                ...
                .ok_or(ProcessContractEventsWithDependenciesError::EventConfigNotFound(
                    dependency.contract_name.clone(),
                    dependency.event_name.clone(),
                ))?;

            if event_processing_config.live_indexing {
                let rindexer_event_filter = event_processing_config.to_event_filter()?;
                let live_indexing_config = LiveIndexingConfig {
                    contract_address: rindexer_event_filter.contract_address(),
                    topic_id: event_processing_config.topic_id,
                    reorg_safe_distance: event_processing_config.indexing_distance_from_head,
                    filter: rindexer_event_filter,
                    info_log_name: event_processing_config.info_log_name.clone(),
                    disable_logs_bloom_checks: event_processing_config
                        .network_contract
                        .disable_logs_bloom_checks,
                    cached_provider: Arc::clone(
                        &event_processing_config.network_contract.cached_provider,
                    ),
                    semaphore: Arc::clone(&event_processing_config.semaphore),
                };
                live_indexing_configs.push(live_indexing_config);
            }
        }
    ...
    // Find the base event processing config - assumes first one is the base
    let base_config = events_processing_config.first().ok_or_else(|| {
        ProcessContractEventsWithDependenciesError::EventConfigNotFound(
            "Base config".to_string(),
            "Base event".to_string(),
        )
    })?;

    process_event_logs(Arc::clone(base_config), Some(live_indexing_configs), true).await?;
